...
<p style="font-size:60%;margin-top:0px;">
...
<p style="font-size:50%;margin-top:0px;">
...
<p style="text-align: center; margin-top:0px; font-size: 110%;">
...
<p style="text-align: center; margin-top:15px; font-size: 80%;">
...
<p style="text-align: center; margin-top:50px; font-size: 80%;">
...
<p style="text-align: center; margin-top:50px; font-size: 60%;">

Pour réaliser cela LDA va proposer deux méthodes :

*Échantillonnage de Gibbs (Gibbs Sampling)* : méthode de Monte Carlo par chaînes de Markov qui échantillonne itérativement les assignations de sujets pour chaque mot. Cette méthode converge généralement mieux.

*Inférence variationnelle bayésienne (Variational Bayes)* : approche déterministe qui reformule l'inférence comme un problème d'optimisation. L'inférence variationnelle est souvent plus efficace que l'échantillonnage de Gibbs pour les grands corpus.

</p>

## Choisir le nombre de topics ?

<p style="text-align: center; margin-top:0px; font-size: 80%;">
...
1- Explorer un nombre adéquat de topics à l'aide du package `ldatunig` qui propose 4 métriques basées sur la littérature (Griffiths 2004, Cao 2009, Arun 2010, Deveaud 2014).

</p>

<p style="text-align: center; margin-top:50px; font-size: 80%;">

2- Etudier la perplexité et la cohérence des topic obtenus.
...
3- Analyse qualitative des topics obtenus pour évaluer leur sens et leur interprétabilité.
...
Ce champs **continue d'évoluer et est très dynamique** !

> les dernières techniques publiées datent de **2024 et 2025**.

> Ces nouvelles méthodes sont très axées sur une intégration de modèle avec des réseaux de neurones ou encore de l'IA.

</p>

<p style="text-align: center; margin-top:50px; font-size: 80%;">

On peut toutefois citer le **correlated topics model** (CTM; Blei and Lafferty 2007) qui est une extension de la méthode LDA où les topics peuvent être corrélés.

Ou encore la méthode **BERTopic** qui intègre le contexte et le sens des mots, méthode développé en 2022 par [Grootendorst](https://arxiv.org/abs/2203.05794) qui va aussi utilisé des méthodes non paramétrique de réduction de dimensions. Cette méthode est de plus en plus employée, très intéressantes et **tant à devenir la méthode de référence**.
...
Les données que nous allons travaillé aujourd'hui : un corpus textuel de qualité sur la Bretagne !

En bref, le contenu brut de 8 pages wikipédia sur des fondamentaux de la région Bretagne!

Notre point de départ peut être :

1- Des fichiers TXT brutes dans un dossier (**notre cas ici**)

2- Un fichier excel

3- Ou encore un fichier avec des mots déjà lemmatisés sortie d'un autre logiciel
...
Si votre corpus texte a déjà fait l'objet d'une analyse par exemple via Iramuteq, il a donc déjà subit un certain nombre d'étape de préparation que nous allons voir maintenant, Dans ce cas vous pouvez vous rendre directement à la case DTM !
