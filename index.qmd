---
title: "Semaine Data SHS 2025"
subtitle: <img src="figure/livre.webp" width="280" style="margin-bottom:0px;"/>
author: "Initiation au Topic Modeling avec R<Br>"
institute: |
  Grégoire Le Campion - 11 décembre 2025<Br>
format:
  revealjs:
    theme: dark
...
1- C'est une méthode qui appartient au champ de l'apprentissage non supervisé et au TAL (ou NLP en anglais).
:::

</br>

::: {style="font-size: 0.8em;margin-top:10px;"}
2- Elle repose sur des modèles probabilistes et il existe plusieurs méthodes de topic modeling basées sur différents algorithmes.
:::

</br>

::: {style="font-size: 0.8em;margin-top:10px;"}
3- C'est une approche complète qui repose sur plusieurs métriques et permet de nombreuses représentations.
:::
...
*1-* Un texte va être composé d'un ou de plusieurs topics ou thèmes. Un texte pourra alors être représenté par une combinaison de ces thèmes.
...
[**Tout l'enjeu du topic modeling va être de capturer cette intuition dans un cadre mathématique**]{style="color:#4178bc;"}
:::

## Sur quoi repose le topic modeling

::: {style="font-size: 0.8em;margin-top:10px;"}
Petite chronologie de l'agorithme derrière le topic modeling :
...
3- **Latent Dirichlet Allocation (LDA)** : La méthode de référence pendant de nombreuses années qui est encore utilisé aujourd'hui. Développé par [Blei, Ng et Jordan](http://www.jmlr.org/papers/volume3/blei03a/blei03a.pdf), publié en 2003.
...
::: {style="font-size: 0.8em;margin-top:10px;"}
[**L'algorithme va donc chercher à estimer deux probabilités essentielles :**]{style="color:#4178bc;"}

[**- La distribution des topics pour chaque texte document.**]{style="color:#4178bc;"}

[**- La distribution des mots pour chaque topic**]{style="color:#4178bc;"}
:::

## Latent Dirichlet Allocation (LDA) II

::: {style="font-size: 0.8em;margin-top:20px;"}
L'algorithme va agir en 3 étapes :
...
## Latent Dirichlet Allocation (LDA) III

::: {style="font-size: 0.8em;margin-top:20px;"}
Pour réaliser cela LDA va proposer deux méthodes :

</br>

[*Échantillonnage de Gibbs (Gibbs Sampling)*]{style="color:#4178bc;"}: méthode de Monte Carlo par chaînes de Markov qui échantillonne itérativement les assignations de sujets pour chaque mot. Cette méthode converge généralement mieux.

</br>

[*Inférence variationnelle bayésienne (Variational Bayes)*]{style="color:#4178bc;"} : approche déterministe qui reformule l'inférence comme un problème d'optimisation. L'inférence variationnelle est souvent plus efficace que l'échantillonnage de Gibbs pour les grands corpus.
:::

## Choisir le nombre de topics ?

::: {style="font-size: 0.8em;margin-top:20px;"}
> Un nombre trop faible et on obtient quelques thèmes très généraux, un trop grand nombre et on va obtenir des topics qui se chevauchent et/ou ininterprétables.
:::

</br>

La solution recommandée est une **approche mixte alliant une démarche quantitative et qualitative**.

</br>

::: {style="font-size: 0.8em;margin-top:10px;"}
[**Point crucial : Les métriques quantitatives guident, mais l'interprétabilité humaine doit avoir le dernier mot. Un modèle avec d'excellentes métriques mais des topics incompréhensibles est inutile en pratique.**]{style="color:#4178bc;"}
:::

## Choisir le nombre de topics ? II

::: {style="font-size: 0.8em;margin-top:10px;"}
1- Etudier la perplexité et la cohérence des topic obtenus.

> La perplexité mesure la capacité prédictive du modèle pour de nouveaux documents. Une perplexité faible indique une meilleure capacité de prédiction. Cependant, la perplexité ne capture pas la cohérence sémantique des sujets.

> La cohérence mesure la qualité sémantique des sujets en comparant la similarité sémantique entre les mots les plus fréquents d'un sujet. Un score de cohérence élevé (proche de 1) indique que les mots d'un sujet sont sémantiquement liés. La cohérence reflète mieux l'interprétabilité humaine des sujets que la perplexité.
:::

## Choisir le nombre de topics ? III

::: {style="font-size: 0.8em;margin-top:10px;"}
2- Analyse qualitative des topics obtenus pour évaluer leur sens et leur interprétabilité.
:::

</br>

::: {style="font-size: 0.8em;margin-top:10px;"}
3- Explorer un nombre adéquat de topics à l'aide du package `ldatunig` qui propose 4 métriques basées sur la littérature (Griffiths 2004, Cao 2009, Arun 2010, Deveaud 2014).
:::

## Conclusion avec la théorie

::: {style="font-size: 0.8em;margin-top:30px;"}
[Ce champ **continue d'évoluer et est très dynamique** !]{style="color:#4178bc;"}
:::

::: {style="font-size: 0.8em;margin-top:20px;"}
On peut toutefois citer :

-   le **correlated topics model** (CTM; Blei and Lafferty 2007) qui est une extension de la méthode LDA où les topics peuvent être corrélés.

-   Ou encore la méthode **BERTopic** qui intègre le contexte et le sens des mots, méthode développée en 2022 par [Grootendorst](https://arxiv.org/abs/2203.05794) qui va aussi utiliser des méthodes non paramétriques de réduction de dimensions. Cette méthode est de plus en plus employée, très intéressantes et **tend à devenir la méthode de référence**.
:::

::: {style="font-size: 0.6em;margin-top:20px;"}
**... mais il faut aller sur Python**
:::
...
::: r-stack
<img src="figure/batman.webp" width="600" style="margin-top:15px;margin-bottom:0px"/>
:::
...
::: {style="font-size: 0.6em;margin-top:20px;"}
Les données que nous allons utiliser aujourd'hui : un corpus textuel de qualité sur la Bretagne !

En bref, le contenu brut de 8 pages wikipédia sur des fondamentaux de la région Bretagne !
:::

::: r-stack
<img src="figure/image.png" width="800" style="margin-top:15px;margin-bottom:0px"/>
:::

## Chargement des données

::: {style="font-size: 0.6em;margin-top:20px;"}
Si votre corpus texte a déjà fait l'objet d'une analyse, par exemple via Iramuteq, il a donc déjà subit un certain nombre d'étapes de préparation que nous allons voir maintenant, Dans ce cas vous pouvez vous rendre directement à la case DTM !
:::
...
# Si vous souhaitez visualiser les 200 premiers caractères du 1er texte
...
::: {style="font-size: 0.6em;margin-top:20px;"}
<img src="figure/alert.png" width="30" style="margin-top:0px;margin-bottom:-7px;margin-right:8px;"/>[**C'est une étape importante de la préparation des données, qui nous permet de passer du corpus au "mot".**]{style="color:#4178bc;"}
:::
...
::: {style="font-size: 0.6em;margin-top:20px;"}
<img src="figure/alert.png" width="30" style="margin-top:0px;margin-bottom:-7px;margin-right:8px;"/>[**C'est une étape sensible et pas forcément nécessaire, à voir en fonction de vos besoins et hypothèses**]{style="color:#4178bc;"}
:::
...
# Sachez qu'il est aussi possible d'utiliser les packages spacyr et udpipe
...
# Si votre corpus en entrée correspond à une base de données avec des variables de type texte, mots, nombre. Comme ce qu'on peut obtenir après certaines analyses réalisées avec certains logiciel de textométrie, vous pouvez utiliser ce code
#   dtm_ira<- data_ira %>% 
...
# Mais en réalité c'est vous qui savez en fonction du matériau utilisé et des hypothèses
...
V(graph_filtered)$degree <- degree(graph_filtered)
...
## Passage au Topic Model !

La première étape va être de choisir le nombre de topics !

``` r
# ===============================================================
# 1. Encore et toujourjours chargement et préparation des données
# ===============================================================
...
## Passage au Topic Model ! 2

Une fois le nombre de topic obtenu on va pouvoir lancer l'analyse :
...
## Analyse et représentation de notre modèle
...
saveRDS(lda_model, paste0("lda_model_k", k_optimal, ".rds"))

# Exporter les termes par topic
write.csv(top_terms, 
          paste0("topics_top_terms_k", k_optimal, ".csv"), 
          row.names = FALSE)

# Exporter l'attribution des documents
write.csv(results_docs, 
          paste0("documents_topics_k", k_optimal, ".csv"), 
          row.names = FALSE)

# Exporter la matrice gamma (probabilités document-topic)
gamma_df <- as.data.frame(lda_model@gamma)
gamma_df$doc_id <- doc_ids
gamma_df$source <- sources
write.csv(gamma_df, 
          paste0("gamma_matrix_k", k_optimal, ".csv"), 
          row.names = FALSE)

# Exporter les probabilités beta (terme-topic)
beta_df <- tidy(lda_model, matrix = "beta")
write.csv(beta_df, 
          paste0("beta_matrix_k", k_optimal, ".csv"), 
          row.names = FALSE)
```
...
``` r
 #==============================================================================
  # 10. Autre visu et test distance, UMAP
  # ==============================================================================
